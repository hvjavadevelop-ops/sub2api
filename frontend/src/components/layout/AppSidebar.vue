<template>
  ...
  >
    ...
    <!-- Navigation -->
    <nav ref="sidebarNavRef" class="sidebar-nav scrollbar-hide" @scroll.passive="rememberSidebarNavScroll">
        ...
        <div class="sidebar-section">
          <template v-for="item in adminNavItems" :key="item.path">
            ...
            <template v-if="item.children?.length">
              ...
              <div v-if="!sidebarCollapsed && isGroupExpanded(item)" class="mb-1 ml-4 border-l border-gray-200 pl-2 dark:border-dark-600">
                <router-link
                  ...
                  :key="child.path"
                  :to="sidebarLinkTarget(child.path)"
            ...
            <component
              ...
              :to="item.externalUrl ? undefined : sidebarLinkTarget(item.path)"
...
const sidebarNavScrollStorageKey = 'sub2api:sidebar-nav-scroll'
let restoringSidebarNavScroll = false
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/models', label: '模型广场', icon: ModelSquareIcon, hideInSimpleMode: true },
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/model-catalog', label: '模型配置', icon: ModelSquareIcon },
    { path: '/admin/announcements', label: t('nav.announcements'), icon: BellIcon },
    { path: '/admin/proxies', label: t('nav.proxies'), icon: ServerIcon },
    { path: '/admin/redeem', label: t('nav.redeemCodes'), icon: TicketIcon, hideInSimpleMode: true },
    { path: '/admin/daily-checkins', label: '签到记录', icon: GiftIcon, hideInSimpleMode: true },
...
function rememberSidebarNavScroll() {
  if (restoringSidebarNavScroll) return
  const scrollTop = sidebarNavRef.value?.scrollTop ?? 0
  sessionStorage.setItem(sidebarNavScrollStorageKey, String(scrollTop))
}

function getSavedSidebarNavScroll(): number {
  const saved = Number(sessionStorage.getItem(sidebarNavScrollStorageKey) || '0')
  return Number.isFinite(saved) ? saved : 0
}

function restoreSidebarNavScroll() {
  if (!sidebarNavRef.value) return
  const savedScrollTop = getSavedSidebarNavScroll()
  restoringSidebarNavScroll = true
  sidebarNavRef.value.scrollTop = savedScrollTop
  requestAnimationFrame(() => {
    if (sidebarNavRef.value) {
      sidebarNavRef.value.scrollTop = savedScrollTop
    }
    restoringSidebarNavScroll = false
  })
}

function sidebarLinkTarget(path: string): RouterLinkProps['to'] {
  return { path, state: { preserveSidebarScroll: true } }
}

function handleMenuItemClick(itemPath: string) {
  rememberSidebarNavScroll()
...
function handleGroupClick(item: NavItem) {
  ...
  if (route.path !== item.path) {
    router.push(sidebarLinkTarget(item.path))
  }
  if (!expandedGroups.value.has(item.path)) {
    expandedGroups.value.add(item.path)
  }
